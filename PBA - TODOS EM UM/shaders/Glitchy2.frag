...
vec2 iResolution = openfl_TextureSize;
    ...
    float colR = texture(iChannel0, vec2(uv.x + shift, uv.y+0.2)).r * (1. - shift) ;
    float colG = texture(iChannel0, vec2(uv.x - shift, uv.y+0.2)).g * (1. - shift) + rand(id) * shift;
    float colB = texture(iChannel0, vec2(uv.x - shift, uv.y+0.2)).b * (1. - shift);
