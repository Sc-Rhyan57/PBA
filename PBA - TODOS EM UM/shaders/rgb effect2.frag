...
{
    return fract(sin(dot(seed * floor(iTime * 10.), vec2(27.1,11.7))) * 43758.5453123);
}

float rng(float seed)
{
    return rng2(vec2(seed, 4.2));
}
...
vec2 iResolution = openfl_TextureSize;
    ...
    vec2 blockS = floor(uv * vec2(14., 9.));
    ...
    vec4 col3 = texture(iChannel0, uv - vec2(lineNoise * 0.05 * rng(5.0), 0));
    
	fragColor = vec4(vec3(col1.x, col2.y, col3.z) + noise, 0.4);
